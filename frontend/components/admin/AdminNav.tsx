...
export function AdminNav() {
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      ...
      <Link href="/admin/categories" className="flex items-center gap-3 rounded-lg border border-black/10 bg-white p-4 shadow-soft transition hover:-translate-y-0.5">
        <div className="grid h-11 w-11 place-items-center rounded-lg bg-coral/10 text-coral">
          <Tags size={22} aria-hidden />
        </div>
        <div>
          <h2 className="font-semibold">Категории</h2>
          <p className="text-sm text-ink/60">Структура каталога</p>
        </div>
      </Link>
    </div>
  );
